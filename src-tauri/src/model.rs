...
// serde derives below are new versus the gpui-era model.rs: these types now
// cross the Tauri IPC boundary as JSON, not just live in one process. The
// `rename_all = "lowercase"` on the three enums keeps IPC and the DB's
// as_str()/parse() convention emitting the same strings, so there is one
// string form of "queued"/"download"/"video", not two that can drift.
