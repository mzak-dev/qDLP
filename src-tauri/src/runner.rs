...
/// `%LOCALAPPDATA%\qDLP\bin` — deliberately not Program Files, so
...
/// that yields `...\Local \qDLP\bin` and fails with ERROR_PATH_NOT_FOUND,
...
pub struct Probe {
    /// "video" or "playlist". `_type` is yt-dlp's own JSON key on the way in;
    /// renaming only the deserialize side keeps our own outbound IPC to the
    /// frontend using the plain field name.
...
// Per-variant rename, not rename_all: the variant names spell out the codec
// pair (MkvH264Aac) while as_str()/the DB and IPC wire format use the short
// container code (mkv) — the two were never going to lowercase into each
// other automatically.
...
// Same tag/content shape as ytdlp::Event, for the same reason: a plain
...
// Shell integration
//
// Relocated from gpui's app.rs, same as domain.rs's functions — small,
// Windows-only, and closest in spirit to this file's existing
// CREATE_NO_WINDOW-aware process spawning, not to any UI concern.
// ---------------------------------------------------------------------------

/// Opens a file with whatever the shell has associated with it. `start` is a
/// cmd builtin, hence `cmd /C`; the empty "" is the window title argument,
/// without which a quoted path is swallowed as the title.
...
pub fn reveal_path(path: &str) {
    ...
    let _ = Command::new("explorer").arg(format!("/select,{native}")).spawn();
}

/// Opens the app-data folder yt-dlp is expected in. Creates it first: the
/// failure this accompanies is often that the directory doesn't resolve, and
/// pointing Explorer at a missing path just produces a second error instead
/// of somewhere to drop the binary.
pub fn open_bin_dir() {
    let dir = bin_dir();
...
mod tests {
    ...
    fn download_a_real_video_end_to_end() {
        ...
        let cover = crate::domain::sibling_thumbnail(&files[0]);
        assert!(
            cover.is_some(),
